...
const page = () => {
  ...
  const handleUpdateBlog = async (e) => {
    e.preventDefault();
    const updatedData = {
      blogObjId: blogToEdit?._id,
      blogImage: blogToEdit?.blogImage,
      headingTitle: blogToEdit?.headingTitle,
      body: blogToEdit?.body,
      isAddedToBanner: blogToEdit?.isAddedToBanner,
    };
    await axios
      .post(
        `${process.env.NEXT_PUBLIC_API_URL}/blog/update-blog`,
        updatedData,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className='w-full h-max flex flex-col p-10 space-y-6'>
      <p className='text-3xl font-bold'>Edit Article</p>
